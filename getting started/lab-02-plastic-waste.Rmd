...
```{r}
p <- ggplot(plastic_waste, aes(x = plastic_waste_per_cap)) +
  geom_histogram() + 
  facet_grid(vars(continent))

p + 
  geom_histogram(binwidth = 0.2, color = "yellow", fill = "gray")
```

```{r}
c <- ggplot(plastic_waste, aes(x = plastic_waste_per_cap)) +
  geom_density() +
  facet_grid(vars(continent))

c +
  geom_density(aes(y = after_stat(count)), binwidth = 0.2, fill = "blue")
```
