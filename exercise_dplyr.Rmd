...
5. Rename `continent` to `Continent`.

```{r}
# compare
# select(.data, newName = oldName)
...
```{r}
gapminder |>
  filter(country %in% c("China", "Brazil") & pop > 10000000)

allowed_countries <- c("China", "Brazil")

gapminder |>
  filter(country %in% allowed_countries & pop > 10000000)
```
...
gapminder |> 
  ...
  mutate(
    ...
    lifeExp_center = lifeExp - mean(lifeExp, na.rm = TRUE)) |> 
  ungroup()
```
